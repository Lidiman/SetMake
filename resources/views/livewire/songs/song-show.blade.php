    ...
    <div class="card bg-gradient-to-br from-surface-900 to-surface-800">
        <div class="flex flex-col md:flex-row gap-6 items-start">
            <div class="w-24 h-24 sm:w-32 sm:h-32 rounded-2xl bg-gradient-to-br from-primary-500/20 to-emerald-600/20 border border-primary-500/30 flex items-center justify-center shrink-0">
                <svg class="w-12 h-12 text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 3v10.55c-.59-.34-1.27-.55-2-.55C7.79 13 6 14.79 6 17s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg>
            </div>
            <div class="flex-1 space-y-4 w-full">
                ...
                @if($song->description)
                    <p class="text-surface-300">{{ $song->description }}</p>
                @endif
                <div class="flex flex-wrap gap-2">
                    @if($song->genre)<span class="badge badge-surface">{{ $song->genre }}</span>@endif
                    @foreach($song->tags as $tag)<span class="badge badge-surface">{{ $tag->name }}</span>@endforeach
                </div>
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-surface-700/50">
                    <div><p class="text-xs text-surface-500 uppercase tracking-wider">Key</p><p class="font-medium text-white">{{ $song->key ?: '--' }}</p></div>
                    <div><p class="text-xs text-surface-500 uppercase tracking-wider">BPM</p><p class="font-medium text-white">{{ $song->bpm ?: '--' }}</p></div>
                    <div><p class="text-xs text-surface-500 uppercase tracking-wider">Duration</p><p class="font-medium text-white">{{ $song->formatted_duration }}</p></div>
                    <div><p class="text-xs text-surface-500 uppercase tracking-wider">Difficulty</p>@if($song->difficulty)<span class="badge badge-{{ $song->difficulty->color() }} mt-1">{{ $song->difficulty->label() }}</span>@else<p class="font-medium text-surface-400">--</p>@endif</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2 space-y-6">
            <div class="card space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="bg-surface-800/50 rounded-xl p-4 border border-surface-700/50"><p class="text-sm text-surface-400 mb-1">Tuning</p><p class="font-medium text-white">{{ $song->tuning ?: 'Standard' }}</p></div>
                    <div class="bg-surface-800/50 rounded-xl p-4 border border-surface-700/50"><p class="text-sm text-surface-400 mb-1">Capo</p><p class="font-medium text-white">{{ $song->capo !== null ? 'Fret ' . $song->capo : 'None' }}</p></div>
                </div>
                @if($song->notes)
                    <div class="bg-surface-800/50 rounded-xl p-4 border border-surface-700/50"><p class="text-sm text-surface-400 mb-2">Notes & Arrangement</p><p class="text-surface-200 whitespace-pre-wrap">{{ $song->notes }}</p></div>
                @endif
            </div>
            ...
            @if($spotifyLink?->spotify_embed_url || $youtubeLink?->youtube_embed_url || $song->audio_path)
                <div class="card space-y-4">
                    ...
                    @if($spotifyLink?->spotify_embed_url)
                        <div class="rounded-xl overflow-hidden bg-surface-800"><iframe src="{{ $spotifyLink->spotify_embed_url }}" width="100%" height="152" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe></div>
                    @endif
                    @if($youtubeLink?->youtube_embed_url)
                        <div class="rounded-xl overflow-hidden aspect-video bg-black relative"><iframe class="absolute inset-0 w-full h-full" src="{{ $youtubeLink->youtube_embed_url }}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>
                    @endif
                    @if($song->audio_path)
                        <div class="audio-player"><p class="text-sm text-surface-400 mb-2">Rehearsal Track</p><audio controls class="w-full"><source src="{{ Storage::url($song->audio_path) }}" type="audio/mpeg"></audio></div>
                    @endif
                </div>
            @endif

            {{-- Attachments --}}
            @livewire('attachments.song-attachments', ['song' => $song], key('attachments-' . $song->id))

            {{-- Checklist --}}
            @livewire('songs.song-checklists', ['song' => $song], key('checklists-' . $song->id))
        </div>
        ...
        <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        @foreach($song->links as $link)
                            <a href="{{ $link->url }}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 p-3 rounded-xl hover:bg-surface-800/50 border border-transparent hover:border-surface-700/50 transition-all group">
                                ...
                                <div class="flex-1 min-w-0"><p class="text-sm font-medium text-white group-hover:text-primary-400 transition-colors">{{ $link->label ?: $link->type->label() }}</p><p class="text-xs text-surface-500 truncate">{{ $link->url }}</p></div>
            ...
            <div class="card">
                    ...
                    <div class="space-y-4">
                        @foreach($song->performances->take(5) as $performance)
                            <div class="relative pl-6 border-l-2 border-surface-800 pb-4 last:pb-0">
                                ...
                                <p class="text-sm font-medium text-white">{{ $performance->performed_at->format('M d, Y') }}</p>
                                <p class="text-xs text-surface-400 mt-1">
                                    @if($performance->setlist)
                                        <a href="{{ route('setlists.show', $performance->setlist) }}" class="hover:text-primary-400 transition-colors" wire:navigate>{{ $performance->setlist->title }}</a>
                                    @else
                ...
                @else
                    <p class="text-surface-500 text-sm text-center py-4">No performance history yet.</p>
                @endif
            </div>

            <div class="text-xs text-surface-500 text-center mt-4">Added by {{ $song->creator->name }} on {{ $song->created_at->format('M d, Y') }}</div>
        </div>
    </div>
</div>
